...
async function ping(host, port = 25565, cb, timeout = 5000, decode = true) {
  const srv = await domain.resolveSrv(host);
  if (srv) {
    host = srv.host;
    port = srv.port;
  }
  ...
  socket.on("data", (data) => {
    ...
    if (packetId === 0) {
      const data = incomingBuffer.slice(
      ...
      try {
        ...
        if (decode && message.forgeData && message.forgeData.d) {
          try {
            const buffer = forge.decode(message.forgeData.d);
            const { mods, channels } = forge.deserialize(buffer);
            message.forgeData.mods = mods;
            message.forgeData.channels = channels;
            delete message.forgeData.d;
          } catch (error) {}
        }
        ...
        message.srvRecord = srv ? { host: srv.host, port: srv.port } : null;
        cb(null, message);
...
export function pingJava(host, options = {}) {
  ...
  const { port = 25565, timeout = 5000, decode = true } = options;
